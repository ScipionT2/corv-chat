...
class TestConfigDefaults:
    ...
    def test_wake_word_default(self):
        import config
        assert config.WAKE_WORD == os.environ.get("JARVIS_WAKE_WORD", "jarvis")

    def test_sample_rate_default(self):
        import config
        assert config.SAMPLE_RATE == 16000 or "JARVIS_SAMPLE_RATE" in os.environ

    def test_ollama_url_default(self):
        import config
        expected = os.environ.get("JARVIS_OLLAMA_URL", "http://localhost:11434")
        assert config.OLLAMA_BASE_URL == expected

    def test_ollama_model_default(self):
        import config
        expected = os.environ.get("JARVIS_OLLAMA_MODEL", "qwen2.5-coder:7b")
        assert config.OLLAMA_MODEL == expected

    def test_whisper_model_default(self):
        import config
        expected = os.environ.get("JARVIS_WHISPER_MODEL", "base.en")
    ...
    def test_max_record_seconds(self):
        import config
        assert config.MAX_RECORD_SECONDS == 30 or "JARVIS_MAX_RECORD_SEC" in os.environ
...
class TestConfigEnvOverride:
    ...
    def test_env_override_wake_word(self, monkeypatch):
    ...
    def test_env_override_invalid_int_uses_default(self, monkeypatch):
        monkeypatch.setenv("JARVIS_SAMPLE_RATE", "not_a_number")
        import config
        importlib.reload(config)
