/**
 * Nova Web Control Hub — Merged & Fixed JS (v2)
 * ──────────────────────────────────────────────
...
(function () {
   ...
   *
   * HTML contract:
   *   • sidebar items have class "nav-item" with attribute data-panel="<name>"
   *   • panels have id "panel-<name>" and class "panel"
   *   • active state uses class "active" on both nav-item and panel
   *   • chat panel: show welcomeScreen OR chatMessages, show chatInputContainer,
   *     show rightPanel (class "visible")
   *   • other panels: hide welcomeScreen, hide chatInputContainer, hide rightPanel
   */
  function switchPanel(target) {
    ...
    // --- Show matching panel (id = "panel-{name}") ---
    ...
    // --- Chat-specific visibility ---
    var welcomeScreen = $('welcomeScreen');
    var chatInputContainer = $('chatInputContainer');
    var rightPanel = $('rightPanel');

    if (panelName === 'chat') {
      // Show chat input and right panel
      if (chatInputContainer) chatInputContainer.style.display = '';
      if (rightPanel) rightPanel.classList.add('visible');
      // Welcome screen is shown until first message; don't forcefully hide it here
      // (it may already be hidden if user has sent messages)
    } else {
      // Non-chat panels: hide welcome, input, and right panel
      if (welcomeScreen) {
  ...
  function init() {
    ...
    // Default active panel: status
    switchPanel('status');

    // Load chat history (shows welcome or past messages)
    loadHistory();
